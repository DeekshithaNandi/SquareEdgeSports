...
import com.squareedgesports.dto.PermissionsDto;
...
public class UserController {
    ...
    public ResponseEntity<?> updateProfile(@RequestBody Map<String, String> body, Authentication auth) {
        ...
        if (body.containsKey("fullName") && body.get("fullName") != null) u.setFullName(body.get("fullName"));
        if (body.containsKey("phone"))        u.setPhone(body.get("phone"));
        if (body.containsKey("addressLine1")) u.setAddressLine1(body.get("addressLine1"));
        if (body.containsKey("addressLine2")) u.setAddressLine2(body.get("addressLine2"));
        if (body.containsKey("city"))         u.setCity(body.get("city"));
        if (body.containsKey("state"))        u.setState(body.get("state"));
        if (body.containsKey("country"))      u.setCountry(body.get("country"));
        if (body.containsKey("zipCode"))      u.setZipCode(body.get("zipCode"));
        if (body.containsKey("profilePicture")) u.setProfilePicture(
            body.get("profilePicture") != null && !body.get("profilePicture").isBlank()
                ? body.get("profilePicture") : null);
        userRepo.save(u);
    ...
    public ResponseEntity<?> uploadPicture(@RequestParam("file") MultipartFile file, Authentication auth) {
        if (file.isEmpty()) return ResponseEntity.badRequest().body(ApiResponse.error("No file provided."));
        ...
        User u = userRepo.findByEmail(auth.getName()).orElseThrow();
        try {
            // Use absolute path so files are always found regardless of working directory
            java.nio.file.Path uploadDir = java.nio.file.Paths.get("uploads/avatars").toAbsolutePath();
            Files.createDirectories(uploadDir);
            String ext = Objects.requireNonNull(file.getOriginalFilename()).replaceAll(".*\\.", "");
            String filename = "avatar_" + u.getId() + "_" + System.currentTimeMillis() + "." + ext;
            Files.copy(file.getInputStream(), uploadDir.resolve(filename), StandardCopyOption.REPLACE_EXISTING);
            u.setProfilePicture("/uploads/avatars/" + filename);
            userRepo.save(u);
            return ResponseEntity.ok(ApiResponse.ok("Picture uploaded", Map.of("url", u.getProfilePicture())));
        } catch (IOException e) {
    ...
    public ResponseEntity<?> myFeedback(Authentication auth) {
        User u = userRepo.findByEmail(auth.getName()).orElseThrow();
        return ResponseEntity.ok(feedbackRepo.findByUserIdOrderByCreatedAtDesc(u.getId()).stream().map(f ->
            Map.of("id", f.getId(), "rating", f.getRating(),
    ...
    public ResponseEntity<?> membershipConfirm(@RequestBody Map<String, String> body, Authentication auth) {
        ...
        switch (sportType != null ? sportType : "") {
            ...
            default -> { return ResponseEntity.badRequest().body(ApiResponse.error("Unknown sportType")); }
        }
